...
public final class FluidGenerator extends DataGenerator_1_19_3<Fluid> {
    ...
    public JsonObject generate() {
        ...
        List<ResourceLocation> fluidRLs = FLUID_REGISTRY.keySet().stream().sorted(Comparator.comparingInt(value -> FLUID_REGISTRY.getId(FLUID_REGISTRY.get(value)))).toList();
        JsonObject fluids = new JsonObject();

        for (ResourceLocation fluidRL : fluidRLs) {
            Fluid f = FLUID_REGISTRY.get(fluidRL);

            JsonObject fluid = new JsonObject();
            fluid.addProperty("id", FLUID_REGISTRY.getId(f));
            fluid.addProperty("mojangName", names.get(f));
            fluid.addProperty("bucketId", ITEM_REGISTRY.getKey(f.getBucket()).toString());
            ...
            }
            {
                ...
                for (final FluidState fs : f.getStateDefinition().getPossibleStates()) {
                    ...
                    state.addProperty("blockState", BLOCK_REGISTRY.getKey(fs.createLegacyBlock().getBlock()).toString());
